...
## [3.6.0](https://github.com/comfino/PrestaShop/tree/3.6.0) (2026-08-18)
...
able to run PHP 7.1+ should move to the 4.x line.

### Added
- Frontend error reporting — the plugin now obtains a short-lived logging token from Comfino and passes it to the SDK, so problems occurring in the customer's browser reach Comfino support instead of being lost.
...
### Changed
- Paywall migrated from the legacy server-rendered iframe to the Comfino V3 API and the new web frontend SDK loaded from Comfino's CDN — faster loading, better reliability and continued compatibility with the payment platform.
- Product page widget migrated to the same SDK: the configuration is now embedded directly in the page and a bridge script is loaded from Comfino's CDN, so no per-request script generated by the shop is involved. All existing widget settings (price and anchor selectors, price change detection, widget type, offer types, embedding method, provider logos, custom banner/calculator CSS) keep working unchanged.
- Loan type and term selected in the paywall are now submitted together with the order form instead of being kept in a cookie, which prevents the order from falling back to a default financing offer. The submitted values are additionally validated against the offers actually available for the cart.
- The Comfino payment method tile in PrestaShop 1.6 checkout now uses the SDK-provided layout, logo and loading state.
- The payment method now disappears from the checkout when the list of available financing options cannot be obtained, instead of showing an option that would fail after the customer selects it.
- Information about available financing options is remembered for a short time instead of being requested on every page view, and requests stop for a few minutes after repeated failures. Product and checkout pages therefore stay fast even when the payment service is slow or unreachable, and the last known list keeps being used in the meantime.
- The log file is no longer kept among the module's files, where a web server could serve it. It now lives with the shop's other logs, is capped in size and rotated, and the payment key and customer data are removed from its contents.
- The method used to verify that settings updates and payment status notifications genuinely come from Comfino now behaves the same way on every supported PHP version, including PHP 5.6, instead of silently falling back to a weaker method on older setups.

### Security
- Redirect addresses returned by the API are validated before use, server variables are sanitized before being logged, and both the widget and the checkout configuration are encoded so that no shop setting can break out of the page markup.
- Payment keys are now entered as hidden fields, are never sent back to the browser once saved (only the last four characters are shown for identification), and are no longer included in messages shown when a key is rejected. Leaving a key field empty keeps the saved key.
- Connections to the payment service now use fixed time limits, always verify the certificate, and are restricted to secure connections. A slow or unresponsive service can no longer hold up a shop page indefinitely.
- The financing option submitted with an order is always checked against the options actually enabled for the cart, and the repayment period is bounded. The order is not created if the list of options cannot be verified.
- Addresses of custom stylesheets that Comfino may set for the widget are now restricted to secure addresses on Comfino's own servers or the shop's own domain.
- The payment error page no longer displays text taken from the address bar, so it cannot be used to show a misleading message on the shop's own domain.
- Direct web access to the module's files is now blocked correctly; the previous rules did not match some of the intended files.

### Removed
- Deprecated V1 paywall integration (`/v1/paywall` iframe URL) and the V2 UMD paywall/widget scripts served from `widget.comfino.pl`.
- The manually editable widget initialization code, along with the widget script version options — the widget is now configured entirely through the regular settings form.
- The `offer` and `script` front controllers, which existed only to serve the removed cookie-based offer capture and the generated widget script.
